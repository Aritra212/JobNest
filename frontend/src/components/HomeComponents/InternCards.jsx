...
function InternCards({ setViewJob, setDetailData }) {
  const { t } = useTranslation(["home"]);
  const internCardDetails = [
    {
  ...
  const fetchInternships = async (category) => {
    setData([]);
    axios
      .get(`${conf.backendURL}/internship/category/${category}/${lang}`)
      .then((res) => setData(res.data.data));
    setActiveCat(category);
  };

  useEffect(() => {
    fetchInternships(internCardDetails[0].title);
  ...
  return (
    <div className="w-11/12 sm:w-4/5 mx-auto my-4">
      <p className="text-xl font-bold mb-4 text-gray-700">
        {t("cardHeadings.internCat")}
      </p>
      <div className="flex gap-2 flex-wrap h-fit">
        {internCardDetails?.map((el) => (
          <p
            key={el.title}
            ...
            onClick={() => fetchInternships(el.title)}
          >
            {el.title}
          </p>
        ))}
